...
exports.postUpdateProfile = (req, res, next) => {
  ...
  User.findById(req.user.id, (err, user) => {
    ...
    user.profile.affiliation = req.body.affiliation || '';
    user.save((err) => {
      if (err) {
        if (err.code === 11000) {
          req.flash('errors', {
...
exports.getVerifyEmail = (req, res, next) => {
  ...
  const sendVerifyEmail = (token) => {
    ...
    const mailOptions = {
      ...
      subject: 'Please verify your email address on Project Strawberry',
      text: `Thank you for registering with Project Strawberry.\n\n
...
exports.postReset = (req, res, next) => {
  ...
  const sendResetPasswordEmail = (user) => {
    ...
    const mailOptions = {
      to: user.email,
      subject: 'Your Project Strawberry password has been changed',
...
exports.postForgot = (req, res, next) => {
  ...
  const sendForgotPasswordEmail = (user) => {
    ...
    const mailOptions = {
      to: user.email,
      from: process.env.SENDGRID_EMAIL,
      subject: 'Reset your password on Project Strawberry',
